...
export default function ProdutosPage() {
  ...
  const filtrados = useMemo(() => {
    ...
    return produtos.filter((p) =>
      [p.codigo, p.nome, p.categoria, p.cor, p.fornecedores?.nome]
        .join(" ")
        .toLowerCase()
        .includes(texto)
    );
      ...
      <div className="flex">
        <aside className="min-h-screen w-64 bg-gradient-to-b from-slate-950 to-blue-900 text-white">
          ...
          <nav className="space-y-2 px-5">
            ...
            <a href="/fornecedores" className="block rounded-xl px-4 py-3 hover:bg-white/10">Fornecedores</a>
            <a href="/produtos" className="block rounded-xl bg-blue-600 px-4 py-3">Produtos</a>
          ...
          <div className="p-8">
            ...
            <section className="rounded-2xl bg-white p-6 shadow-sm">
              <div className="mb-5 flex items-center justify-between gap-4">
                <h3 className="text-xl font-bold text-slate-800">Produtos cadastrados</h3>
                <input
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-sm">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {filtrados.map((p) => (
                      <tr key={p.id}>
                        <td className="px-4 py-4 font-semibold text-slate-800">{p.nome}</td>
                        <td className="px-4 py-4">{p.fornecedores?.nome}</td>
                        <td className="px-4 py-4">{p.categoria}</td>
                        <td className="px-4 py-4">R$ {Number(p.preco || 0).toFixed(2)}</td>
                        <td className="px-4 py-4">{p.comissao_percentual}%</td>
                        <td className="space-x-2 px-4 py-4">
                          <button onClick={() => setForm({ ...p, preco: String(p.preco), comissao_percentual: String(p.comissao_percentual) })} className="rounded-lg border px-3 py-2 hover:bg-slate-50">
                            Editar
                          </button>
                          <button onClick={() => excluir(p.id)} className="rounded-lg bg-red-100 px-3 py-2 text-red-700">
                            Excluir
                          </button>
                        </td>
                      </tr>
