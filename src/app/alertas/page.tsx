...
export default function AlertasPage() {
  ...
  const carregar = useCallback(async () => {
    ...
    const [clientes, visitas, receber, pagar, pipeline, comissoes, metas, pedidos, cobrancas, resolvidosResp] = await Promise.all([
      ...
      supabase.from("contas_pagar").select("id, descricao, fornecedor, valor, vencimento, status").neq("status", "Pago"),
    ...
    setAlertas(gerarAlertasInteligentes({
      ...
      contasPagar: ((pagar.data || []) as Linha[]).map((item) => ({ id: String(item.id), titulo: texto(item.descricao), cliente: texto(item.fornecedor), valor: numero(item.valor), data: texto(item.vencimento), status: texto(item.status) })),
