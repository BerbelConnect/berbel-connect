...
export default function ComissoesFinanceiroPage() {
  ...
  async function carregarComissoes() {
    ...
    const { data, error } = await supabase.from("comissoes_financeiro")
      .select("id, pedido_id, created_at, empresa, percentual, valor_base, valor_comissao, previsao_recebimento, data_previsao, data_recebimento, data_pagamento_cliente, regra_recebimento, status, clientes(razao_social), pedidos(numero, status)")
    ...
    if (error) setErro(error.message);
      ...
      return {
        ...
        recebimento: texto(item.data_recebimento) || null, pagamento_cliente: texto(item.data_pagamento_cliente) || null,
        regra_recebimento: texto(item.regra_recebimento) || null, status: texto(item.status) || "Pendente",
      };
    }));
    setCarregando(false);
  }
  ...
  async function informarPagamentoCliente(item: ComissaoFechamento) {
    const data = prompt("Data em que o cliente pagou (AAAA-MM-DD):", hoje());
    if (data === null) return;
    if (!/^\d{4}-\d{2}-\d{2}$/.test(data)) return alert("Informe a data no formato AAAA-MM-DD.");
    const { error } = await supabase.from("comissoes_financeiro").update({ data_pagamento_cliente: data }).eq("id", item.id);
    if (error) return alert(error.message);
    await carregarComissoes();
    alert("Pagamento do cliente registrado. A previsão foi calculada para o próximo dia útil aplicável.");
  }
  ...
  return <main className="min-h-screen bg-slate-100"><div className="flex"><Sidebar /><section className="min-w-0 flex-1">
    ...
    <div className="space-y-5 p-4 md:p-8">
      <section className="rounded-2xl bg-white p-5 shadow-sm"><div className="grid gap-4 md:grid-cols-3 xl:grid-cols-7">
        ...
        <Campo titulo="Situação"><select value={situacao} onChange={(e) => setSituacao(e.target.value)} className="mt-2 w-full rounded-xl border border-slate-300 p-3"><option value="">Todas</option><option>Aguardando cliente</option><option>Pendente</option><option>Vencida</option><option>Recebida</option></select></Campo>
      ...
      <section className="rounded-2xl bg-white p-5 shadow-sm"><h2 className="mb-4 text-xl font-bold">Comissões detalhadas</h2><div className="overflow-x-auto"><table className="min-w-full text-left text-sm"><thead className="bg-slate-100"><tr><th className="p-3">Representada</th><th className="p-3">Cliente</th><th className="p-3">Pedido</th><th className="p-3">Regra / previsão</th><th className="p-3 text-right">Base</th><th className="p-3 text-right">Comissão</th><th className="p-3">Situação</th><th className="p-3">Ação</th></tr></thead><tbody className="divide-y">{filtradas.map((item) => <tr key={item.id}><td className="p-3 font-semibold">{item.empresa}</td><td className="p-3">{item.cliente}</td><td className="p-3">{item.pedido}</td><td className="p-3"><div>{dataBr(item.previsao)}</div><div className="mt-1 max-w-xs text-xs text-slate-500">{item.regra_recebimento || descricaoRegraComissao(item.empresa)}</div></td><td className="p-3 text-right">{moeda(item.valor_base)}</td><td className="p-3 text-right font-semibold text-emerald-700">{moeda(item.valor_comissao)}</td><td className="p-3"><Badge situacao={item.situacao} /></td><td className="p-3">{item.situacao === "Recebida" ? <button onClick={() => void estornar(item.id)} className="rounded-lg bg-amber-100 px-3 py-2 text-amber-800">Estornar</button> : item.situacao === "Aguardando cliente" ? <button onClick={() => void informarPagamentoCliente(item)} className="rounded-lg bg-blue-600 px-3 py-2 font-semibold text-white">Informar pagamento</button> : <button onClick={() => setBaixa({ id: item.id, data: hoje(), forma: "Transferência", motivo: "Comissão recebida" })} className="rounded-lg bg-emerald-600 px-3 py-2 font-semibold text-white">Receber</button>}</td></tr>)}{!carregando && filtradas.length === 0 ? <tr><td colSpan={8} className="p-8 text-center text-slate-500">Nenhuma comissão encontrada.</td></tr> : null}</tbody></table></div></section>
    </div>
...
function Badge({ situacao }: { situacao: string }) { const cor = situacao === "Recebida" ? "bg-emerald-100 text-emerald-700" : situacao === "Vencida" ? "bg-red-100 text-red-700" : situacao === "Aguardando cliente" ? "bg-blue-100 text-blue-700" : "bg-amber-100 text-amber-700"; return <span className={`whitespace-nowrap rounded-full px-3 py-1 text-xs font-bold ${cor}`}>{situacao}</span>; }
